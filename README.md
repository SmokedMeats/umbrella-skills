...
It also pulls methods from [Lauren Tan (poteto)](https://github.com/poteto) / pstack: `/how`, `/why`, `/teach` (explain), `/principles` (one index), and `/blast-radius`. Agents auto-invoke them when the work needs them. A slash menu is not required. After tickets are filed, overnight `/loop` is optional. This overlay does not install poteto-mode or the full pstack plugin.
...
Understand and rigor: [Lauren Tan (poteto)](https://github.com/poteto) / pstack. `/how`, `/why`, `/teach`, the `/principles` index, and `/blast-radius` are adapted from pstack, with house unslop.
