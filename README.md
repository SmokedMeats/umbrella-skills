...
| `code-review` | After the two-axis report, **Close the loop**: each Standards hard violation and Spec missing/partial/wrong becomes remaining ACs on the **same** tickets, then `/implement` again. `Later:` only for grill/spec not-this-pack. A report without that remaining-AC verdict is unfinished. Flag a hand-parsed bag or one-off retry only next to an existing Effect sibling — not `Effect.gen` for its own sake. |
