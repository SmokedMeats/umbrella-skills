...
| Skill | Change |
| --- | --- |
| `umbrella` | New conductor. Every run: if Matt's pack or the repo mapping is missing, install / run setup first. Catch includes unhoused maps/children (no `umbrella:*`), open issues with no milestone, and open issues missing from the XyberRun Project, and hands them to `/triage`. After any create, `Later:` / `Leftover:` tickets must carry **When to do this** ([PARKED-TICKETS.md](skills/umbrella/PARKED-TICKETS.md)). After any close, [CLOSE-PARENTS.md](skills/umbrella/CLOSE-PARENTS.md) (close the parent when open children = 0, including parked). After product-done Device QA, [DEVICE-QA.md](skills/umbrella/DEVICE-QA.md) (`adb` probe; no phone + last leftover → **Desk device** + notes, do not close). Pulling a ticket assigns `@me` on GitHub first and sets Project Status **In Progress**. Two or more unblocked tickets → `/implement` wave. Later waves crawl as they unlock. Build loop includes a **skip-unless** Effect-TS check (see `/implement`). One board: [PROJECTS.md](skills/umbrella/PROJECTS.md) (Kanban / Roadmap / Houses / Now / Grill / Parked). Resolve a card via `issue.projectItems` — do not `item-list` the first 200. |
